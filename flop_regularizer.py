...
# choose and initialise MorphNet regularizer   
...
# import the generated inverted pendulum data
def import_dataset():
...
def neural_network():
    ...
    morphnet = flopregularizer([model.output.op], threshold=1e-3)
    ...
    save_model(model, y_pred2, x_test, "flop_model.h5")
    ...
    plt.ylabel('loss')
    plt.xlabel('epoch')
    plt.legend(['seed network', 'FLOP optimized network'], loc='upper center')
    plt.figure()
    ...
    plt.xlabel('inputs')
    plt.ylabel('targets')
    plt.legend(['seed network', 'FLOP Optimized Network' ], loc='lower right')
    plt.show()
